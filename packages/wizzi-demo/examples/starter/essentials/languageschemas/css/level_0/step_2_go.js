/*
    ...
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi\packages\wizzi-demo\.wizzi\examples\starter\essentials\languageschemas\css\level_0\step_2_go.js.ittf
*/
'use strict';
...
function heading1(text) {
    ...
    console.log('** level 0 - step 2 - language_schema_css - ' + text);
    console.log('*'.repeat(120));
    console.log('');
}
function heading2(text) {
    ...
    console.log('   ','-- language_schema_css - ' + text);
    console.log('   ', '-'.repeat(100));
    console.log('');
}
...
module.exports = function exec(step_callback) {
    ...
    heading1('starting step 2 cssFlex.css.ittf')
    wizzi.gen(path.join(__dirname, 'step_2', 'cssFlex.css.ittf'), {}, {
        globalContext: {}, 
        artifactContext: {}, 
        ...
        var dest = path.join(__dirname, 'step_2', 'outputs', verify.stripExtension('cssFlex.css.ittf'));
        wizzi.file.write(dest, artifactText)
        console.log(verify.stripExtension('cssFlex.css.ittf') + ' saved to ' + dest);
        printValue('artifactText', artifactText)
        heading1('starting step 2 cssFlex.html.ittf')
        wizzi.gen(path.join(__dirname, 'step_2', 'cssFlex.html.ittf'), {}, {
            globalContext: {}, 
            artifactContext: {}, 
            ...
            // Save the artifact
            // to the ./outputs folder
            var dest = path.join(__dirname, 'step_2', 'outputs', verify.stripExtension('cssFlex.html.ittf'));
            wizzi.file.write(dest, artifactText)
            console.log(verify.stripExtension('cssFlex.html.ittf') + ' saved to ' + dest);
            printValue('artifactText', artifactText)
            step_callback(null);
        })
    })
